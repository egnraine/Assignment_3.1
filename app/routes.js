...
<><p>var feedSchema = new mongoose.Schema({feedURL}: {type}: String, trim:true },
    ...
    {collection}: 'feed' }
    );</p><p>feedSchema.index({feedURL}: 1}, {unique}:true});
        feedSchema.index({link}: 1}, {unique}:true, sparse:true});</p><p>var FeedModel = mongoose.model( 'Feed', feedSchema );</p><p>var feedEntrySchema = new mongoose.Schema({description}: {type}: String, trim:true },
        ...
        );</p><p>feedEntrySchema.index({entryID} : 1});
            feedEntrySchema.index({feedID} : 1});</p><p>var FeedEntryModel = mongoose.model( 'FeedEntry', feedEntrySchema );</p><p>var userFeedEntrySchema = new mongoose.Schema({userID}: {type}: mongoose.Schema.Types.ObjectId },
